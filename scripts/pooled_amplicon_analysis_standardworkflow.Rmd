...
knitr::opts_knit$set(root.dir = "~/Documents/Postdoc/benchmark_demulticoder")
...
taxa_its<- read.table("standard_workflow/its/outputs/its_taxa.out", sep="\t", stringsAsFactors=F,header=T)
...
#Load ASV matrix now
seqtab_nochim_its <- read.table("standard_workflow/its/outputs/its_seqtab_nochim.out")
...
#Save file
write.csv(combined_df_its, "standard_workflow/its/outputs/its_abundance_matrix_combined_dada2workflow.csv")
```

### Track number of reads throughout the pipeline
```{r track reads through DADA2 analysis its, include=TRUE}
track_reads_standard_wf_its <- read.table("standard_workflow/its/outputs/its_trackreads.out", sep="\t", stringsAsFactors = F, header=T)
```

### Let's also do the same for the rps10 dataset
```{r load files standard workflow rps10}
taxa_rps10<- read.table("standard_workflow/rps10/outputs/rps10_taxa.out", sep="\t", stringsAsFactors=F,header=T)
...
#Load ASV matrix now
seqtab_nochim_rps10 <- read.table("standard_workflow/rps10/outputs/rps10_seqtab_nochim.out")
...
#Save file
write.csv(combined_df_rps10, "standard_workflow/rps10/outputs/rps10_abundance_matrix_combined_dada2workflow.csv")
```

### Track number of reads throughout the pipeline
```{r track reads through DADA2 analysis rps10, include=TRUE}
track_reads_standard_wf_rps10 <- read.table("standard_workflow/rps10/outputs/rps10_trackreads.out", sep="\t", stringsAsFactors = F, header=T)
```
...
metadata_path <- file.path("standard_workflow/combined_data/metadata_comp.csv")
...
rps10_database <- read_fasta("standard_workflow/rps10/data/rps10_reference_db_unique.fa")
