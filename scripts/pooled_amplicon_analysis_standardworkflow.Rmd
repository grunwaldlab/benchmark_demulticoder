...
### For the sample data, we add columns for the proportion of reads in each sample representing each inoculum as well as a columns that indicate whether the expected inoculum was found, and if so, if it was the only inoculum found.
```{r}
...
#Results are arbitrarily mirrored compared to my demulticoder analyses, so I will fix this here, without changing hte meaning
...
# Rename metadata$flooded values from TRUE FALSE to Flooded Not Flooded
...
# export diff_table_flood of only the significant taxa
write.csv(obj_subset$data$diff_table_flood[obj_subset$data$diff_table_flood$padj < 0.05,], file = "standard_workflow/cobined_results/diff_table_flood.csv")
...
obj_subset %>%
  ...
  remove_redundant_names() %>%
   heat_tree_matrix(data = "diff_table_flood",
            ...
            node_color_trans = "linear",
            node_color_interval = c(-6.5, 6.5),
            edge_color_interval = c(-6.5, 6.5),
            ...
            node_size_axis_label = "Mean read proportion",
            node_color_axis_label = "Log2 fold change",
            output_file = file.path('standard_workflow/combined_figures', 'diff_taxon_abund_flood.pdf'))
```
