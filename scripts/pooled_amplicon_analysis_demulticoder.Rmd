...
### Reload ASV matrices and metadata for final checks when 
```{r load reads}
seed <- 1
set.seed(seed)
...
obj_newdb %>%
  filter_taxa(! is_stem) %>%
  filter_taxa(n_samples >= 3, supertaxa = TRUE, reassign_obs = FALSE) %>%
  filter_taxa(! grepl(x = taxon_names, "_sp$"), reassign_obs = FALSE) %>%
  filter_taxa(! grepl(x = taxon_names, "incertae_sedis", ignore.case = TRUE), reassign_obs = FALSE) %>%
  remove_redundant_names() %>%
  heat_tree(node_size = mean_prop,
...
obj_olddb %>%
  filter_taxa(! is_stem) %>%
  filter_taxa(n_samples >= 5, supertaxa = TRUE, reassign_obs = FALSE) %>%
  filter_taxa(! grepl(x = taxon_names, "_sp$"), reassign_obs = FALSE) %>%
  filter_taxa(! grepl(x = taxon_names, "incertae_sedis", ignore.case = TRUE), reassign_obs = FALSE) %>%
  remove_redundant_names() %>%
  heat_tree(node_size = mean_prop,
...
old_db <- image_read_pdf("demulticoder/figures/heattree_mostabund_taxa_rps10_olddb.pdf")
new_db <- image_read_pdf("demulticoder/figures/heattree_mostabund_taxa_rps10_newdb.pdf")
...
old_grob <- rasterGrob(as.raster(old_db_resized))
new_grob <- rasterGrob(as.raster(new_db_resized))

pdf("demulticoder/figures/combined_heat_trees_db_compare.pdf", width=12, height=8)
grid.newpage()
