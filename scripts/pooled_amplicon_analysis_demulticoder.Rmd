...
rps10_database <- read_fasta("demulticoder/outputs/rps10_reference_db.fa")
...
obj_newdb %>%
  ...
  heat_tree(node_size = mean_prop,
            ...
            edge_size_digits = 2,
            #aspect_ratio = 1.618,
            output_file = file.path('demulticoder/figures', '/heattree_mostabund_taxa_rps10_newdb_boot0_greater1.svg'))
```
...
obj_olddb %>%
  ...
  heat_tree(node_size = mean_prop,
            ...
            edge_size_digits = 2,
            #aspect_ratio = 1.618,
            output_file = file.path('demulticoder/figures', '/heattree_mostabund_taxa_rps10_olddb_boot0_greater1.svg'))
```

Let's facet the two plots so we have a side-by-side comparison of the two databases. 
```{r, facet community plot, message=FALSE, echo=FALSE}
old_db <- image_read_pdf("demulticoder/figures/heattree_mostabund_taxa_rps10_olddb_boot0_greater1.pdf")
new_db <- image_read_pdf("demulticoder/figures/heattree_mostabund_taxa_rps10_newdb_boot0_greater1.pdf")

old_grob <- rasterGrob(as.raster(old_db))
new_grob <- rasterGrob(as.raster(new_db))

### Now try saving the plot using cowplot
combined_heat_trees <- plot_grid(old_grob, new_grob, ncol = 2, labels = c('A.', 'B.'), 
                                 rel_heights = c(1, 1))

# Adjust the layout to reduce white space and adjust margins
combined_heat_trees <- combined_heat_trees + 
  theme(plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"))

# Save the plot with the specified dimensions
ggsave(combined_heat_trees, path = "demulticoder/figures", filename = "combined_heat_trees_db_compare_60boot.pdf", 
       width = 7, height = 4.5, bg = "#FFFFFF")

ggsave(combined_heat_trees, path = "demulticoder/figures", filename = "combined_heat_trees_db_compare_0boot.svg", 
       width = 7, height = 4.5, bg = "#FFFFFF")
```
