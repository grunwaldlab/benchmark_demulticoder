...
```{r prepare reads}
asv_matrix_rps10<-read.csv("demulticoder/outputs/final_asv_abundance_matrix_rps10.csv")
...
asv_matrix_rps10$dada2_tax <- gsub(";oomycetedb_[0-9]+--[0-9]+--Reference", "", asv_matrix_rps10$dada2_tax)

asv_matrix_its<-read.csv("demulticoder/outputs/final_asv_abundance_matrix_its.csv")
...
write.csv(abundance, "demulticoder/outputs/final_asv_abundance_matrix_combined_demulticoder.csv")

metadata_path <- file.path("demulticoder/data/metadata_comp.csv")
metadata <- read_csv(metadata_path)
print(metadata)

#Fourth, let's track reads
track_reads_demulticoder_its<-read.csv("demulticoder/outputs/track_reads_its.csv", row.names = 1)
track_reads_demulticoder_rps10<-read.csv("demulticoder/outputs/track_reads_rps10.csv", row.names = 1)
```
...
rps10_database <- read_fasta("demulticoder/data/rps10_reference_db_unique.fa")
...
aligned_data_path <- file.path("demulticoder/outputs", "infection_aligned_data.rds")
...
For the sample data, we will add columns for the proportion of reads in each sample representing each inoculum as well as a columns that indicate whether the expected inoculum was found, and if so, if it was the only inoculum found.
...
write_csv(abundance, file.path('demulticoder/outputs', 'abundance_with_infection_data.csv'))
...
# Save new metadata
write_csv(metadata, file.path('demulticoder/outputs', 'metadata_with_infection_data.csv'))
```
...
### Let's make new taxmap object in preparation to make heattree examining abundance and diversity of taxa, and filter out ASVS not present at least 5 times across samples
...
#                    seed=1000,
#                    key_size = 0.5,
#                    output_file = file.path('demulticoder/figures', 'diff_taxon_abund_conc.pdf'))
```
...
```{r abundance plots-flood, fig.height=30, fig.width=30, message=FALSE, echo=FALSE}
# Calculate difference between groups
set.seed = 1000
...
obj_subset %>%
  ...
  heat_tree_matrix(data = "diff_table_flood",
            ...
            output_file = file.path('demulticoder/figures', 'diff_taxon_abund_flood.svg'))
```
...
#                    seed=1000,
#                    key_size = 0.5,
#                    output_file = file.path('demulticoder/figures', 'diff_taxon_abund_organism.pdf'))
# ```
# 
...
#             node_color = ifelse(is.na(padj) | padj > 0.05, 0, log2FoldChange), # difference between groups
#             node_color_trans = "linear",
#             node_color_interval = c(-27, 27),
#             edge_color_interval = c(-27, 27),
#             node_size_range = c(.005, .03),
#             node_size_trans = "log10",
#             node_label_size_range = c(.015, .02),
#             node_color_range = diverging_palette(),
#             node_size_axis_label = "Mean read proportion",
#             node_color_axis_label = "Log2 fold change",
#             output_file = file.path('demulticoder/figures', 'diff_taxon_abund_trial.pdf'))
```

### Let's try to facet these plots. 
```{r facet abundance plots, fig.height=30, fig.width=30, message=FALSE, echo=FALSE}
# # Read the images
# conc_img <- magick::image_read_pdf("demulticoder/figures/diff_taxon_abund_conc.pdf")
# trial_img <- magick::image_read_pdf("demulticoder/figures/diff_taxon_abund_trial.pdf")
# flood_img <- magick::image_read_pdf("demulticoder/figures/diff_taxon_abund_flood.pdf")
# org_img <- magick::image_read_pdf("demulticoder/figures/diff_taxon_abund_organism.pdf")
# 
# # Convert to grobs
...
# pdf("demulticoder/figures/combined_heat_trees.pdf", width=12, height=10)
# grid.newpage()
...
### Let's finally make an exploratory heat tree to look at community composition across all samples, just retaining ASVs present in at least 50 samples

### Overall community plot
```{r, overall community plot 2}
obj_subset <- obj2 %>%
  filter_taxa(taxon_ranks == "Species", supertaxa = TRUE, reassign_obs = c(tax_abund = FALSE)) 
min_bootstrap <- 60
...
obj_subset %>%
  ...
  filter_taxa(! grepl(x = taxon_names, "_sp$"), reassign_obs = FALSE) %>%
  filter_taxa(! grepl(x = taxon_names, "Branch03"), reassign_obs = FALSE) %>%
  filter_taxa(! grepl(x = taxon_names, "NA"), reassign_obs = FALSE) %>%
  ...
  heat_tree(node_size = mean_prop,
            ...
            edge_size_digits = 2,
            #aspect_ratio = 1.618,
            output_file = file.path('demulticoder/figures', '/heattree_mostabund_taxa.svg'))
```

### Reload ASV matrices and metadata for final checks when examining new db to the old version before updates-only don't filter out ASVs assigned as inoculum species
```{r load reads-compare old and new db}
asv_matrix_rps10_oldb<-read.csv("demulticoder/outputs_olddb/final_asv_abundance_matrix_rps10.csv")
...
asv_matrix_rps10_oldb$dada2_tax <- gsub(";oomycetedb_[0-9]+--[0-9]+--Reference", "", asv_matrix_rps10_oldb$dada2_tax)
...
obj_newdb %>%
  ...
  heat_tree(node_size = mean_prop,
            ...
            edge_size_digits = 2,
            #aspect_ratio = 1.618,
            output_file = file.path('demulticoder/figures', '/heattree_mostabund_taxa_rps10_newdb_boot60_greater1.svg'))
```
...
obj_olddb %>%
  ...
  heat_tree(node_size = mean_prop,
            ...
            edge_size_digits = 2,
            #aspect_ratio = 1.618,
            output_file = file.path('demulticoder/figures', '/heattree_mostabund_taxa_rps10_olddb_boot60_greater1.svg'))
```
...
extract_first_diff <- function(old_boot, new_boot) {
  old_values <- unlist(strsplit(old_boot, ";"))
  new_values <- unlist(strsplit(new_boot, ";"))
...
merged_taxa$first_diff_boot_newdb <- mapply(extract_first_diff, merged_taxa$dada2_tax_old_boot, merged_taxa$dada2_tax_new_boot)
merged_taxa$first_diff_boot_olddb <- mapply(extract_first_diff, merged_taxa$dada2_tax_new_boot, merged_taxa$dada2_tax_old_boot)

merged_taxa$comparison <- mapply(function(x, y) {
  if (is.na(x) & is.na(y)) {
    return("equal")
...
}, merged_taxa$first_diff_boot_newdb, merged_taxa$first_diff_boot_olddb)

total_comparisons <- sum(!is.na(merged_taxa$comparison))
prop_higher_newdb <- sum(merged_taxa$comparison == "higher", na.rm = TRUE) / total_comparisons
prop_less_newdb <- sum(merged_taxa$comparison == "less", na.rm = TRUE) / total_comparisons
prop_equal_newdb <- sum(merged_taxa$comparison == "equal", na.rm = TRUE) / total_comparisons

write.csv(merged_taxa, "demulticoder/results/taxdifferences_oldnewdb.csv")

cat("Proportion of times new db value is higher than old db value:", prop_higher_newdb, "\n")
cat("Proportion of times new db value is less than old db value:", prop_less_newdb, "\n")
cat("Proportion of times new db value is equal to old db value:", prop_equal_newdb, "\n")
```
