...
abundance<-read.csv("demulticoder/data/final_asv_abundance_matrix_combined.csv")
...
obj_subset %>%
  ...
  heat_tree(node_label = taxon_names,
            ...
            edge_color_interval = c(-5.15, 5.15),
...
place_grob(conc_grob, 3, 2, "D. Pathogen Concentration")

dev.off()

# SVG version
svg("demulticoder/figures/combined_heat_trees.svg", width=12, height=10)
grid.newpage()
pushViewport(viewport(layout = grid.layout(3, 2, 
                                           heights = unit(c(0.1, 1, 1.5), "null"),
                                           widths = unit(c(1, 1), "null"))))

# Repeat the same placement for SVG
place_grob(flood_grob, 2, 1, "A. Flooding Status")
place_grob(trial_grob, 2, 2, "B. Trial")
place_grob(org_grob, 3, 1, "C. Organism")
