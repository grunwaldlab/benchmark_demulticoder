...
# add associated taxonoimc assignments before exporting table data16S_nowf_separate <- data16S_nowf %>% 
  group_by(Treatuntreat_EP, Kingdom, Phylum) %>% 
  summarise(Abundance = sum(Abundance), Prevalence = n_distinct(Sample)) %>% 
  ungroup() %>% 
  group_by(Treatuntreat_EP) %>% 
  mutate(Total_Abundance = sum(Abundance)) %>% 
  ungroup() %>% 
  mutate(Prop = Abundance / Total_Abundance)

data16S_nowf_separate_filt <- data16S_nowf_separate %>%
  filter(Prop > 0.01)

write.csv(obj_subset$data$diff_table_flood[obj_subset$data$diff_table_flood$padj < 0.05,], file = "demulticoder/results/diff_table_flood.csv")
...
obj_subset %>%
  ...
  heat_tree_matrix(data = "diff_table_flood",
            ...
            node_color_trans = "linear",
            node_color_interval = c(-5.2, 5.2),
            edge_color_interval = c(-5.2, 5.2),
            ...
            output_file = file.path('demulticoder/figures', 'diff_taxon_abund_flood.pdf'))
```
...
ggsave(combined_heat_trees, path = "demulticoder/figures", filename = "combined_heat_trees_db_compare_0boot.pdf", 
