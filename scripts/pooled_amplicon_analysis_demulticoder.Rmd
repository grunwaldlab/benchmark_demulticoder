---
title: "Pooled amplicon analysis-demulticoder outputs"
...
seed <- 1
set.seed(seed)
#Let's start by loading CSV files containing matrices themselves
asv_matrix_rps10<-read.csv("demulticoder/final_asv_abundance_matrix_rps10.csv")
asv_matrix_its<-read.csv("demulticoder/final_asv_abundance_matrix_its.csv")
...
abundance<-read.csv("demulticoder/final_asv_abundance_matrix_combined.csv")

metadata_path <- file.path("demulticoder/metadata.csv")
...
rps10_database <- read_fasta("demulticoder/rps10_reference_db.fa")
...
aligned_data_path <- file.path("demulticoder", "infection_aligned_data.rds")
...
write_csv(abundance, file.path('demulticoder', 'abundance_with_infection_data.csv'))
...
# Save new metadata
write_csv(metadata, file.path('demulticoder', 'metadata_with_infection_data.csv'))
```
...
metadata$richness <- vegan::specnumber(prob_table, MARGIN = 2)
metadata$shannon <- vegan::diversity(prob_table, MARGIN = 2, index = "shannon")
metadata$invsimpson <- vegan::diversity(prob_table, MARGIN = 2, index = "invsimpson")
write.csv(metadata, file.path("demulticoder", "alpha_diversity.csv"))
print(metadata)
...
ggsave(nmds_plot, path = "figures", filename = "nmds.pdf", 
       width = 7, height = 8)
print(nmds_plot)
...
ggsave(combined_div_plot, path = "figures", filename = "diversity.pdf", 
       width = 10, height = 6, bg = "#FFFFFF")

ggsave(combined_div_plot, path = "figures", filename = "diversity.svg", 
       width = 10, height = 6, bg = "#FFFFFF")
```

### Let's make new taxmap object in preparation to make heattree examining abundance and diversity of taxa in at least 10 samples
...
### Let's make differential abundance plots for our combined dataset-first let's look at diff. abundance of taxa by inoculum
...
significant_taxon_ids <- unique(
  obj_subset$data$diff_table_conc$taxon_id[! is.na(obj_subset$data$diff_table_conc$padj) & obj_subset$data$diff_table_conc$padj < 0.1]
)
...
obj_subset %>%
  ...
  heat_tree_matrix(data = "diff_table_conc",
                   ...
                   seed=1000,
                   key_size = 0.5,
                   output_file = file.path('figures', 'diff_taxon_abund_conc.pdf'))
```
...
significant_taxon_ids <- unique(
  obj_subset$data$diff_table_flood$taxon_id[! is.na(obj_subset$data$diff_table_flood$padj) & obj_subset$data$diff_table_flood$padj < 0.1]
)
...
obj_subset %>%
  ...
  heat_tree(node_label = taxon_names,
            ...
            node_size_axis_label = "Mean read proportion",
            node_color_axis_label = "Log2 fold change",
            output_file = file.path('figures', 'diff_taxon_abund_flood.pdf'))
```
...
significant_taxon_ids <- unique(
  obj_subset$data$diff_table_org$taxon_id[! is.na(obj_subset$data$diff_table_org$padj) & obj_subset$data$diff_table_org$padj < 0.1]
)
...
obj_subset %>%
  ...
  heat_tree_matrix(data = "diff_table_org",
                   ...
                   seed=1000,
                   key_size = 0.5,
                   output_file = file.path('figures', 'diff_taxon_abund_organism.pdf'))
```

### Let's try to facet these plots. 
```{r facet abundance plots, fig.height=30, fig.width=30, message=FALSE, echo=FALSE}
conc_img <- magick::image_read_pdf("figures/diff_taxon_abund_conc.pdf")
conc_grob <- grid::rasterGrob(image = as.raster(conc_img))

flood_img <- magick::image_read_pdf("figures/diff_taxon_abund_flood.pdf")
flood_grob <- grid::rasterGrob(image = as.raster(flood_img))

org_img <- magick::image_read_pdf("figures/diff_taxon_abund_organism.pdf")
org_grob <- grid::rasterGrob(image = as.raster(org_img))
...
ggsave(
  "figures/combined_heat_trees.pdf",
  combined_figure,
  width = 12,
  height = 24,
  units = "in",
...
#Let's also make SVG figure
ggsave(
  "figures/combined_heat_trees.svg",
  combined_figure,
  width = 12,
  height = 24,
  units = "in",
...
obj_subset %>%
  ...
  heat_tree(node_size = mean_prop,
            ...
            edge_size_digits = 2,
            #aspect_ratio = 1.618,
            output_file = file.path('figures', '/supp_figure_2_whole_community.pdf'))
```
