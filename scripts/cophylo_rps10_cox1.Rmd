...
### Read in the data
```{r, read in trees}
cox1<-ape::read.tree("rps10_cox1_phylogeny/data/cox1_db_mafft_auto.fasta.treefile")
#cox1<-midpoint(cox1)
rps10<-read.tree("rps10_cox1_phylogeny/data/rps10_db_mafft_unique_gp1000_ugene.fasta.treefile")
...
# Define genus colors
# Define genus colors
genus_colors <- setNames(c("#009E73", "#56B4E9","#F0E442", "#D55E00"), unique(datasettable$genus))
...
pdf("rps10_cox1_phylogeny/figures/figure_cox_vs_rps10_auto_rotatemultitrue.pdf", width = 8.5, height = 11, useDingbats = FALSE)
plot(obj, tip.len = 0.01, fsize = 0.20, scale.bar = c(0.2, 0.2), pts = FALSE, link.type="curved", link.lwd=1, link.lty="solid", link.col=make.transparent("black", 0.4))
title(main = "Cophylogeny", line = -1)

# Add tip labels with colors
tiplabels.cophylo(pch = 21, bg = cox1_tip_colors, cex = 0.7, which = "left")
tiplabels.cophylo(pch = 21, bg = rps10_tip_colors, cex = 0.7, which = "right")
