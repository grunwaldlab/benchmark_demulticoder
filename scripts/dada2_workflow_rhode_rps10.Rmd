...
knitr::opts_knit$set(root.dir = "~/demulticoder_benchmarking_2/rhode_DADA2_workflow")
...
### Workflow is similar to what is described in the rps10 tutorial in DADA2 documentation, but since amplicons were pooled, I need to first demultiplex rps10 reads within sample, or else rps10 reads will still be included in all downstream steps
...
process_file_pair <- function(i) {
  ...
  # Keep reads where either forward or reverse read contains an rps10 primer
...
fnFs.prefilt <- file.path(path, "rps10/prefiltered", basename(fnFs))
fnRs.prefilt <- file.path(path, "rps10/prefiltered", basename(fnRs))

out <- filterAndTrim(fnFs.demultiplex, fnFs.prefilt, fnRs.demultiplex, fnRs.prefilt, maxN=0, compress=TRUE, multithread=TRUE)
```
...
write.table(seqtab.nochim, file="outputs/rps10/rps10_seqtab_nochim.out",sep="\t",quote=F)
```
...
write.table(track, file="outputs/rps10/rps10_trackreads.out",sep="\t",quote=F)
```

## If everything look good, let's assign taxonomy
``` {r assignTaxonomy}
oomycetedb.ref <- "db/rps10_reference_db.fa"  # CHANGE ME to location on your machine
...
write.table(taxa, file="outputs/rps10/rps10_taxa.out",sep="\t",quote=F)
```
