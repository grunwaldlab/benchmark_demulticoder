...
#Add to new table
nmds_df <- data.frame(sample_name, expected_innoc,nmds1_demulticoder, nmds1_sf, nmds1_diff, nmds2_demulticoder, nmds2_sf, nmds2_diff)
...
make_one_plot <- function(factor, name) {
  nmds_data %>%
    ...
    scale_x_continuous(labels = scales::number_format(accuracy = 0.1)) +  # Format x-axis labels
...
demulticoder_flood <- image_read_pdf("demulticoder/figures/diff_taxon_abund_flood.pdf")
sf_flood <- image_read_pdf("standard_workflow/combined_figures/diff_taxon_abund_flood.pdf")
...
combined_heat_trees <- plot_grid(demulticoder_grob, sf_grob, ncol = 2, labels = c('A.', 'B.'), 
                                 rel_heights = c(1, 1))
...
### DESEQ2-flood-flood-difference_comparison
```{r facet figure-deseq continued}
#In progress
```

### Heat trees of most abundant taxa-side-by-side comparison
```{r facet figure-most abundant taxa}
demulticoder_flood <- image_read_pdf("demulticoder/figures/heattree_mostabund_taxa.pdf")
sf_flood <- image_read_pdf("standard_workflow/combined_figures/heattree_mostabund_taxa.pdf")
...
### Heat trees of most abundant taxa-side-by-side comparison
```{r most abundant taxa cont}
#Alt approach in progress
```
