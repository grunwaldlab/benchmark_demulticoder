...
richness_diff <- stats_diversity_sf$richness - stats_diversity_demulticoder$richness 
shannon_demulticoder <- stats_diversity_demulticoder$shannon
shannon_sf <- stats_diversity_sf$shannon
shannon_diff <- stats_diversity_sf$shannon - stats_diversity_demulticoder$shannon  
invsimpson_demulticoder <- stats_diversity_demulticoder$invsimpson
invsimpson_sf <- stats_diversity_sf$invsimpson
invsimpson_diff <- stats_diversity_sf$invsimpson - stats_diversity_demulticoder$invsimpson
...
nmds1_diff <- - nmds_sf$NMDS1 - nmds_demulticoder$NMDS1 
nmds2_diff <- - nmds_sf$NMDS2 - nmds_demulticoder$NMDS2 
...
### DESEQ2-flood side-by-side
I need to adjust the dimensions
```{r facetted figure-deseq}
demulticoder_flood <- magick::image_read_pdf("demulticoder/figures/diff_taxon_abund_flood.pdf")
sf_flood <- magick::image_read_pdf("standard_workflow/combined_figures/diff_taxon_abund_flood.pdf")

sf_grob <- rasterGrob(as.raster(sf_flood))
demulticoder_grob <- rasterGrob(as.raster(demulticoder_flood))

### Now try saving the plot using cowplot
combined_heat_trees <- cowplot::plot_grid(demulticoder_grob, sf_grob, ncol = 2, labels = c('A.', 'B.'), 
...
# Save the plot with the specified dimensions
ggsave(combined_heat_trees, path = "demulticoder_comp_sf/figures", filename = "combined_heat_trees_deseq2_flood.pdf", 
       width =7, height = 4, bg = "#FFFFFF")

ggsave(combined_heat_trees, path = "demulticoder_comp_sf/figures", filename = "combined_heat_trees_deseq2_flood.svg", 
       width =7, height = 4, bg = "#FFFFFF")
```


### Heat trees of most abundant taxa-side-by-side comparison
```{r facet figure-most abundant taxa}
demulticoder_abund <- magick::image_read_pdf("demulticoder/figures/heattree_mostabund_taxa.pdf")
sf_abund <- magick::image_read_pdf("standard_workflow/combined_figures/heattree_mostabund_taxa.pdf")

sf_grob <- rasterGrob(as.raster(sf_abund))
demulticoder_grob <- rasterGrob(as.raster(demulticoder_abund))
...
# Save the plot with the specified dimensions
ggsave(combined_heat_trees, path = "demulticoder_comp_sf/figures", filename = "mostabund_taxa_combined.pdf", 
       width =7, height = 4, bg = "#FFFFFF")

ggsave(combined_heat_trees, path = "demulticoder_comp_sf/figures", filename = "mostabund_taxa_combined.svg", 
       width =7, height = 4, bg = "#FFFFFF")
```
